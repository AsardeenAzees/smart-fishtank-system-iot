...
// Modes
enum Mode { MODE_AUTO, MODE_MANUAL, MODE_OFF };
Mode lightMode = MODE_AUTO, pumpMode = MODE_AUTO;

// ---- Buzzer mode (defined BEFORE use) ----
enum BuzzerMode { BUZZER_NORMAL, BUZZER_FEED_ONLY };
...
void setLight(bool on){ lightOn = on; relayWrite(PIN_LIGHT_RELAY, lightOn); }
void setPump (bool on){ pumpOn  = on; relayWrite(PIN_PUMP_RELAY,  pumpOn ); }
void setBuzzer(bool on){ buzzerOn = on; digitalWrite(PIN_BUZZER, on ? HIGH : LOW); }
...
void feederDispense(){
  ...
  if (buzzerMode == BUZZER_FEED_ONLY) beepMs(200);
...
Mode strToMode(const String& s){
  String a = s; a.toLowerCase();
  if (a == "manual") return MODE_MANUAL;
  if (a == "off")    return MODE_OFF;
  return MODE_AUTO;
}
...
bool isSameDay(time_t a, time_t b){
  struct tm la, lb; localtime_r(&a, &la); localtime_r(&b, &lb);
...
int parseHHMMtoSec(const String& hhmm){
  int colon = hhmm.indexOf(':'); if (colon < 0) return -1;
...
bool jsonGetBool  (FirebaseJson& j, const char* path, bool  &out){
  FirebaseJsonData r; if (j.get(r, path) && r.success && r.typeNum == FirebaseJson::JSON_BOOL){ out = r.to<bool>(); return true; } return false;
}
bool jsonGetInt   (FirebaseJson& j, const char* path, int   &out){
  FirebaseJsonData r; if (j.get(r, path) && r.success && (r.typeNum==FirebaseJson::JSON_INT||r.typeNum==FirebaseJson::JSON_FLOAT||r.typeNum==FirebaseJson::JSON_DOUBLE)){ out = r.to<int>(); return true; } return false;
}
bool jsonGetFloat (FirebaseJson& j, const char* path, float &out){
  FirebaseJsonData r; if (j.get(r, path) && r.success && (r.typeNum==FirebaseJson::JSON_INT||r.typeNum==FirebaseJson::JSON_FLOAT||r.typeNum==FirebaseJson::JSON_DOUBLE)){ out = (float)r.to<double>(); return true; } return false;
}
bool jsonGetString(FirebaseJson& j, const char* path, String& out){
  FirebaseJsonData r; if (j.get(r, path) && r.success && r.typeNum == FirebaseJson::JSON_STRING){ out = r.to<const char*>(); return true; } return false;
}

void fetchSettings(){
  if (!Firebase.ready()) return;
  if (Firebase.RTDB.getJSON(&fbdo, PATH_SETTINGS)){
    FirebaseJson* jp = fbdo.to<FirebaseJson*>(); if (!jp) return; FirebaseJson& j = *jp;

    bool b; int i; float f; String s;

    if (jsonGetBool (j, "presence_rule", b))          settings.presenceRuleEnabled = b;
    if (jsonGetBool (j, "low_light_rule", b))         settings.lowLightRuleEnabled = b;
    if (jsonGetBool (j, "hourly_pump/enabled", b))    settings.hourlyPump.enabled  = b;
    if (jsonGetInt  (j, "hourly_pump/seconds", i))    settings.hourlyPump.seconds  = i;

    if (jsonGetInt  (j, "thresholds/proximity_cm",  i)) settings.proximity_cm  = i;
    if (jsonGetInt  (j, "thresholds/low_light_adc", i)) settings.low_light_adc = i;
    if (jsonGetInt  (j, "thresholds/water_low_adc", i)) settings.water_low_adc = i;
    if (jsonGetFloat(j, "thresholds/high_temp_c",   f)) settings.high_temp_c   = f;

    if (jsonGetBool  (j, "feeding/enabled", b)) settings.feedingEnabled = b;
    if (jsonGetString(j, "feeding/time1", s))  settings.feedTime1 = s;
    if (jsonGetString(j, "feeding/time2", s))  settings.feedTime2 = s;

    if (jsonGetBool  (j, "buzzer/alerts_enabled", b)) settings.alertsEnabled = b;

    if (jsonGetString(j, "light_mode", s)) lightMode = strToMode(s);
    if (jsonGetString(j, "pump_mode",  s)) pumpMode  = strToMode(s);

    // Optional default buzzer mode from settings
    if (jsonGetString(j, "buzzer/mode", s)) buzzerMode = strToBuzzerMode(s);
  }
}
...
void applyCommandsSnapshot(){
  ...
  if (Firebase.RTDB.getJSON(&fbdo, PATH_COMMANDS)){
    ...
    if (jsonGetString(j, "light_mode", s)) lightMode = strToMode(s);
    if (jsonGetBool  (j, "light_manual_state", b) && lightMode == MODE_MANUAL) setLight(b);

    if (jsonGetString(j, "pump_mode",  s)) pumpMode  = strToMode(s);
    if (jsonGetBool  (j, "pump_manual_state",  b) && pumpMode  == MODE_MANUAL) setPump(b);

    // NEW: buzzer mode command
    if (jsonGetString(j, "buzzer_mode", s)) buzzerMode = strToBuzzerMode(s);

    if (jsonGetBool(j, "buzzer_toggle", b) && b){
      setBuzzer(!buzzerOn);
      FirebaseJson ack; ack.add("buzzer_toggle", false);
    ...
    if (jsonGetInt(j, "buzzer_silence_minutes", i) && i > 0){
      ...
      FirebaseJson ack; ack.add("buzzer_silence_minutes", 0);
      Firebase.RTDB.updateNode(&fbdo, PATH_COMMANDS, &ack);
    }
    ...
    if (jsonGetBool(j, "feed_now", b) && b){
      ...
      FirebaseJson ack; ack.add("feed_now", false);
      Firebase.RTDB.updateNode(&fbdo, PATH_COMMANDS, &ack);
    }
  }
}
...
void drawOLED(){
  ...
  display.print("T:"); display.print(isnan(gTempC) ? 0 : gTempC, 1);
  display.print("C  H:"); display.print(isnan(gHum) ? 0 : gHum, 0); display.println("%");

  display.setCursor(0, 10);
  display.print("LDR:"); display.print(gLightADC);
  display.setCursor(64, 10);
  display.print("Water:"); display.print(gWaterADC);

  display.setCursor(0, 20);
  display.print("Prox: "); display.print(gProximity); display.println("cm");

  display.setCursor(0, 30);
  display.print("Light:"); display.print(lightOn ? "ON " : "OFF");
  display.print(" ("); display.print(modeToStr(lightMode)); display.print(")");

  display.setCursor(0, 40);
  display.print("Pump :"); display.print(pumpOn  ? "ON " : "OFF");
  display.print(" ("); display.print(modeToStr(pumpMode)); display.print(")");

  display.setCursor(0, 50);
  display.print("Buzz: "); display.print(buzzerOn ? "ON" : "OFF");
...
void applyAutomation(){
  ...
  // LIGHT
  if      (lightMode == MODE_OFF)    setLight(false);
  else if (lightMode == MODE_MANUAL) { /* manual */ }
  else {
  ...
  // PUMP
  if      (pumpMode == MODE_OFF)     setPump(false);
  else if (pumpMode == MODE_MANUAL)  { /* manual */ }
  else {
    bool shouldOn = (settings.presenceRuleEnabled && presence);
    if (!hourlyPumpRunning) setPump(shouldOn); // hourly pump overrides separately
  }
  ...
  if (buzzerMode == BUZZER_NORMAL && settings.alertsEnabled && !withinSilence){
    if (waterLow)      beepMs(2000); // 2 sec chirp
    else if (tempHigh) beepMs(200);  // short beep
  }
}

// Hourly pump worker
void hourlyPumpTick(){
  if (!settings.hourlyPump.enabled || pumpMode != MODE_AUTO) return;
...
void feedingTick(){
  if (!settings.feedingEnabled || !ntpReady) return;
...
void setup(){
  ...
  setLight(false); setPump(false); setBuzzer(false);
